...
void MyGlWindow::update(int updateRate)
{
	...
	for (rep = 0; rep < updateRate; ++rep) {
		...
		for (i = 0; i < size; ++i) {
			if(m_mover[i]->active == false) {
				m_mover[i]->active = true;
				break;
			}
		}
		/* 인력 START */
		...
		//if(false)
			for (i = 0; i < size; ++i) {
				if (m_mover[i]->active)
					m_mover[i]->velocity.y += -5.0;
			}
			// 모든	 공 루프 (자신)
			for (i = 0; i < size; ++i) {
				// 활성화 상태일 경우
				if (m_mover[i]->active) {
					glm::f64vec3 posVec(m_mover[i]->m_position.x, m_mover[i]->m_position.y, m_mover[i]->m_position.z);
					if (
						glm::length(posVec) > 400.0f
						) {
						m_mover[i]->velocity = m_mover[i]->velocity + glm::length(m_mover[i]->velocity) * glm::normalize(zeroVec - posVec);
					}
				}
			}
		/* 인력 END */
						...
						if (i != j && m_mover[j]->active) { // 자신이 아닐경우 & 상대가 활성화 상태인지
							...
							if (m_mover[i]->ballMass >= m_mover[j]->ballMass) {
								...
								if (distance < m_mover[i]->ballSize + m_mover[j]->ballSize) {
									tempVec = m_mover[j]->velocity;
									m_mover[j]->velocity = m_mover[i]->velocity;
									m_mover[i]->velocity = tempVec;
